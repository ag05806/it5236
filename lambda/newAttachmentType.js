						...
						if (err) {
							...
							if(err.errno == 1062) {
								...
								if(err.sqlMessage.indexOf('name') != -1) {
									// This should be a "Internal Server Error" error
									callback(formatErrorResponse('BAD_REQUEST', ["name already exists"]));
								} else if(err.sqlMessage.indexOf('attachmenttypeid') != -1) {
									// This should be a "Internal Server Error" error
									callback(formatErrorResponse('BAD_REQUEST', ["Attachment type incorrect"]));
								} else {
									// This should be a "Internal Server Error" error
