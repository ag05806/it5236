...
exports.handler = (event, context, callback) => {
	...
	conn.connect(function(err) {
		...
		conn.query(sql, [event.thingregistrationcode, event.thingid, event.thingcreated, event.thingname, event.thingattachmentid, event.thinguserid], function (err, result) {
		  	if (err) {
				// This should be a "Internal Server Error" error
				callback(formatErrorResponse('INTERNAL_SERVER_ERROR', [err]));
		  	} else {
					...
					conn.query(sql, [event.thingregistrationcode, event.thingid, event.thingcreated, event.thingname, event.thingattachmentid, event.thinguserid], function (err, result) {
						if (err) {
							...
							if(err.errno == 1062) {
								...
								if(err.sqlMessage.indexOf('thingname') != -1) {
									// This should be a "Internal Server Error" error
									callback(formatErrorResponse('BAD_REQUEST', ["Please choose another name"]));
								} else if(err.sqlMessage.indexOf('email') != -1) {
									// This should be a "Internal Server Error" error
									callback(formatErrorResponse('BAD_REQUEST', ["Email error"]));
								} else {
									// This should be a "Internal Server Error" error
