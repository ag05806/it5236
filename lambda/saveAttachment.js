...
exports.handler = (event, context, callback) => {
  ...
  conn.connect(function(err) {
		...
		var sql = "SELECT COUNT(*) AS codecount FROM attachments WHERE LOWER(attachmentid) = LOWER(?)  ";
		...
		conn.query(sql, [event.attachmentid, event.filename], function (err, result) {
		  	if (err) {
		    	...
		    	console.log("Attachment count is " + result[0].codecount);
		    	if (result[0].codecount == 0){
		    		errors.push("Attachment unavailable");
					callback(formatErrorResponse('BAD_REQUEST', errors));
		    	} else {
