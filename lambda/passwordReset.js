						...
						if (err) {
							...
							if(err.errno == 1062) {
								...
								if(err.sqlMessage.indexOf('username') != -1) {
									// This should be a "Internal Server Error" error
									callback(formatErrorResponse('BAD_REQUEST', ["Username Incorrect"]));
								} else if(err.sqlMessage.indexOf('email') != -1) {
									// This should be a "Internal Server Error" error
									callback(formatErrorResponse('BAD_REQUEST', ["Email address incorrect"]));
								} else {
									// This should be a "Internal Server Error" error
				      		...
				      		var sql = "SELECT email, userid FROM users WHERE username = ? OR email = ?";
								...
								if (err) {
						        	...
						        	console.log("successful reset");
					      			callback(null,"password has been reset");
				      			}
		      				}); 
			      		} 
		    			}); 
